...
if __name__ == "__main__":
    ...
    for b in range(40):
        ...
        if not os.path.exists(qapath):
            os.mkdir(qapath)
        if b < 10:
            datapath = '/data/apertif/{0:s}/{1:02d}/raw/'.format(
                args.taskID, b)
        if 10 <= b < 20:
            datapath = '/data2/apertif/{0:s}/{1:02d}/raw/'.format(
                args.taskID, b)
        if 20 <= b < 30:
            datapath = '/data3/apertif/{0:s}/{1:02d}/raw/'.format(
                args.taskID, b)
        if 30 <= b < 40:
            datapath = '/data4/apertif/{0:s}/{1:02d}/raw/'.format(
                args.taskID, b)
