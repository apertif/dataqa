...
if __name__ == "__main__":
    ...
    def copy_data(beams, n, data):
        for i in beams:
            if os.path.isdir('/'+str(data)+'/apertif/'+str(data_dir)+'/'+str(i)) == True and n in i:
                os.system('cp -r /'+str(data)+'/apertif/'+str(data_dir)+'/'+str(i) +
                          '/continuum/image_mf_*.fits /data/apertif/'+str(data_dir)+'/mosaic/cont_tmp/image_mf_'+str(i)+'.fits')
    ...
    def convert_fits():
        for i in range(len(items)):
            ...
            fits.in_ = '/data/apertif/' + \
                str(data_dir)+'/mosaic/cont_tmp/'+str(items[i])
            fits.out = '/data/apertif/' + \
                str(data_dir)+'/mosaic/cont_tmp/'+str(items[i][:-5])+'.mir'
            fits.op = 'xyin'
            fits.go()
    ...
    linmos.in_ = '/data/apertif/' + \
        str(data_dir)+'/mosaic/cont_tmp/image_mf_*.mir'
    linmos.out = '/data/apertif/' + \
        str(data_dir)+'/mosaic/'+str(data_dir)+'_mosaic_image'
    linmos.go()
    ...
    fits.op = 'xyout'
    fits.out = '/data/apertif/' + \
        str(data_dir)+'/mosaic/'+str(data_dir)+'_mosaic_image.fits'
    fits.go()
